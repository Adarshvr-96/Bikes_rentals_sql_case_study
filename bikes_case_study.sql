...
--1. Emily would like to know how many bikes the shop owns by category. Canyou get this for her?
...
-- customer_name
--count of membership purchased
-- sort by membership count desc
...
from membership m right join customer c on c.id = m.customer_id
 group by c.name
  ...
  price_per_day as old_price_per_day,
    case when category = 'electric' then round(price_per_day -(price_per_day*0.2), 2)
	     when category = 'mountain bike' then round(price_per_day -(price_per_day*0.5), 2)
	           else round(price_per_day - (price_per_day*0.5), 2) end as discounted_price_per_day
from bike 
order by id
...
-- Sort the results chronologically. Display the year total after all the month totals for the corresponding year. 
...
--- total rev from membership 
--- year, month, and membership type
...
--- date 2023
--- group by membership_type and month
--- subtotals and grant totals
--- total revenue
...
-- 8. Now it's time for the final task. Emily wants to segment customers based on the number of rentals and see the count of customers in each segment. Use your SQL skills to get this!

-- Categorize customers based on their rental history as follows: Customers who have had more than 10 rentals are categorized as 'more than 10' .
